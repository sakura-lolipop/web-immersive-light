/* ============================================================
  ...
  零依赖,复制本文件+index.html 的 <style> 受光段即可接入任何页面。

  接入清单:
    1. <style> 里拷 :root 配方族 + #light-layer + .lit/.lit-row 三段
    2. HTML 加 <div id="light-layer"></div>
    3. 受光面元素挂 class="lit"(表行/列表行再挂 lit-row)
    4. 本文件 <script src> 引入(或内联)
    5. SEL 注册表按页面改(下文一行)
...
/* 受光面注册表(名单单一真相):CSS 只认 .lit/.lit-row 单类。
   ...
   接入新页面只改这一行。 */
var SEL = '.card, .modal-content, .toast, .nav-link, .avatar, .list-group-item, td, .input-icon';
